...
{   
    if(save->open(QIODevice::ReadOnly | QIODevice::Text)) {
        ...
        while(!ts.atEnd()) {
            ...
            if(split.size() > 4) {
                map[split[0]] << new AlcoItem({split[1],split[2],split[3].toInt(),split[4].toInt()}, split[0]);
            }
        }
        save->close();
    }   
...
bool AlcoSaveWorker::contains(const QString &value)
{
...
void AlcoSaveWorker::setMap(const AlcoMap &value)
{
    map = value;
}

AlcoMap *AlcoSaveWorker::getMap()
{
    return &map;
}
